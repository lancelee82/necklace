...
class SliceSampler(Sampler):
    def __init__(self, length=None, indices=None, shuffle=False):
    ...
    def __getslice__(self, i, j):  # not used in py3
        return self.indices[i:j]

    def __getitem__(self, idx):  # NOTE: in py3 this is also used when getslice
        list_getitem_fn = list.__getitem__
        return list_getitem_fn(list(self.indices), idx)
...
class SliceBatchSampler(BatchSampler):
    ...
    def shuffle(self):
        if isinstance(self._sampler, (list, tuple, set)):
            random.shuffle(self._sampler)
        else:
