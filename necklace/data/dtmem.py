"""DataLoader using Shared Memory"""
import copy
import random
import time
...
# shared memory based on posix_ipc and pickle
# NOTE: the pickle and unpickle will cost a long time
import shmem as shdnda
...
class DataLoaderSharedMemory(object):
    ...
    def init_data_shm(self, shape, typ, name):
        ...
        size = int(np.prod(self.batch_data_shape)) * np.dtype(typ).itemsize * self.batch_size
        self.data_shm = [shdnda.SharedMemArray(name, obj_size=size, obj_cnt=self.part_num) for i in range(self._num_workers)]
    ...
    def init_label_shm(self, shape, typ, name):
        ...
        size = int(np.prod(self.batch_label_shape)) * np.dtype(typ).itemsize * self.batch_size
        self.label_shm = [shdnda.SharedMemArray(name, obj_size=size, obj_cnt=self.part_num) for i in range(self._num_workers)]
