...
class TrainerMPOPPytorch(tnopbs.TrainerOPBase):
    ...
    def __init__(self, cfg):
        ...
        self.init_params_recv()  # for nccl_grads_allreduce for DP
    ...
    def check_msg(self, msg, *args, **kwargs):
        ...
        # ---------------------------------------------------------------------
            ...
            # =====================================================================
            # NOTE: here we check if it is end of an epoch by that if the inputs
            #       is None, there may be better methods such as by len(dataloader)
            # =====================================================================
            if inputs is None:
                ret = {
            ...
            if self.is_last_part():  # to_rank == self.rank
                ...
                # ====================================================================
                # TODO: should NOT do opt step here ??? [=========]
                # NOTE: ---> after nccl_all_reduce
                # ====================================================================
            ...
            #self.do_a_train_step()  # NOTE: ---> after nccl_all_reduce
            #self.i += 1
    ...
    def do_grads_nccl_all_reduce(self, *args, **kwargs):
        pg_dp = ncclgrp.get_nccl_group_dp_or_main()
        self.cuda_sync()
        ptprms.opt_grads_do_nccl_allreduce(
            #None, self.kn, self.opt, self.prm_recvs,
            pg_dp, pg_dp.world_size, self.opt, self.prm_recvs,
