"""PyTorch Trainer for One Process (ZeRO) (with TsMem flat buffer)"""
#
# BASE: ./zroppf.py
#
# NOTE:
...
class TrainerMPOPPytorch(tnopbs.TrainerOPBase):
    ...
    def __init__(self, cfg):
        ...
        self.gpu_dev_i = cfg.get('gpu_dev_i', torch.cuda.current_device())
    ...
    def init_net(self, msg=None, *args, **kwargs):
        ...
        self.nets = []
        self.opts = []
        self.weig_total_sizes = []
        self.grad_total_sizes = []

        for i, mdl in enumerate(mdls):
            #net = mdwrp.MdWrpWithCkpt(submdls=[mdl,], md_device=self.gpu_i, mp_order=i)  # TODO: [==================]
            is_last_part = i == (len(mdls) - 1)
            net = mdwrp.MdWrpWithSelfBkwd(submdls=[mdl,], md_device=self.gpu_i,
                                          mp_order=i, is_last_part=is_last_part,
                                          #do_acts_ckpt=False)  # TODO: now we use cuda_fltn_mem_mng without checkpointing [=========]
                                          do_acts_ckpt=True)  # TODO: without checkpointing will use more memory [=========]
            #net.to(self.ctx)  # NOTE: now should on CPU
            net.train()  # TODO: do at a better place ? [==================]
            self.nets.append(net)

            weig_total_size = tsmem.md_params_numel_total_size(net)
            #grad_total_size = tsmem.md_grads_numel_total_size(net)  # TODO: p.grad is None  [==============]
            grad_total_size = weig_total_size
            self.weig_total_sizes.append(weig_total_size)
            self.grad_total_sizes.append(grad_total_size)

        self.max_weig_total_size = max(self.weig_total_sizes)
        self.max_grad_total_size = max(self.grad_total_sizes)

        self.cuda_fltn_mem_mng = tsmem.PtCudaFltnMemManager(
            rank=self.rank,
            ctx=self.ctx,
            max_weig_total_size=self.max_weig_total_size,
            #max_grad_total_size=self.max_grad_total_size,
            max_grad_total_size=self.max_weig_total_size,  # TODO: now max_grad_total_size maybe 0 [==============]
        )

        for i, net in enumerate(self.nets):
            self.cuda_fltn_mem_mng.md_to_cpu(net)

            # TODO: after grad_to_gpu [=====================================]
            """
            self.cuda_fltn_mem_mng.md_to_gpu(net)
    ...
    def check_msg(self, msg, *args, **kwargs):
        ...
        # ---------------------------------------------------------------------
            ...
            self.cuda_fltn_mem_mng.md_to_gpu(self.net)

            #self.do_params_nccl_broadcast()
            self.do_params_fltn_nccl_broadcast()
            ...
            if not self.is_last_part():  # keep the last mod in GPU for calc loss
                ...
                self.cuda_fltn_mem_mng.md_to_cpu(self.net)
                self.net = None
            ...
            self.net = self.nets[one_zr_step_order_i]
            #self.net = self.net.cuda()  # to GPU
            ##self.cuda_fltn_mem_mng.md_to_gpu(self.net)  # NOTE:

            self.opt = self.opts[one_zr_step_order_i]  # TODO: maybe no need for now [=========]
            if self.opt == None:
                self.opt = self.cre_one_opt(self.net) # NOTE: now on gpu
                self.opts[one_zr_step_order_i] = self.opt

            #self.do_grads_nccl_reduce_to_me()
            self.do_grads_fltn_nccl_reduce_to_me()
            ...
            #self.opts_zero_grad(self.opts)  # TODO: very slow, why ? [======]
            self.cuda_fltn_mem_mng.zero_grad()
    ...
    def do_params_fltn_nccl_broadcast(self, *args, **kwargs):
        pg = ncclgrp.get_nccl_group_zr_or_main()  # NOTE:
        self.cuda_sync()
        d_arr_send = self.cuda_fltn_mem_mng.prm_fltn_gpu
        sz = self.cuda_fltn_mem_mng.max_weig_total_size
        ptprms.params_fltn_do_nccl_broadcast(
            pg, pg.world_size, self.net_map_order, d_arr_send, sz)
        self.cuda_sync()
    ...
    def do_grads_fltn_nccl_reduce_to_me(self, *args, **kwargs):
        pg = ncclgrp.get_nccl_group_zr_or_main()  # NOTE:
        self.cuda_sync()
        d_arr_send = self.cuda_fltn_mem_mng.grd_fltn_gpu
        sz = self.cuda_fltn_mem_mng.max_grad_total_size
        ptprms.opt_grads_fltn_do_nccl_reduce(
            pg, pg.world_size, self.net_map_order, d_arr_send, sz,
            grad_comp_cfg=self.grad_comp_cfg)
        self.cuda_sync()
