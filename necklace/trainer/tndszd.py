"""ZeRO Trainer Services (Scheduler + Worker)"""
...
ILOG_I_ZR_SVR_WKR = 800001
ILOG_I_ZR_SVC_WKR = 800002
ILOG_I_ZR_SVR_SCH = 800003
ILOG_I_ZR_SVC_SCH = 800004

Q_STATE_HEAD_ZR_WKR = 810000
Q_STATE_HEAD_ZR_SCH = 820000
...
class MPSVRWorker(svctmp.TmplServer):
    ...
    def fn_one_zr_step_start(self, msg, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_WKR, '>>> fn_one_zr_step_start')
        m = self.msg_unpck(msg)
        msg = m.get('msg') or {}

        self.q_put_state(Q_STATE_HEAD_ZR_WKR + 201, msg)

        rep = self.mk_rpc_ret('fn_one_zr_step_start', 'ok')
        return self.msg_pck(rep)

    def do_one_zr_step_ack(self, ret, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_WKR, '>>> do_one_zr_step_ack')
        r = self.do_rpc_call(
            self.scheduler_cli, 'fn_one_zr_step_ack',
            ret=ret)
        return r

    def do_one_zr_step_end(self, ret, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_WKR, '>>> do_one_zr_step_end')
        r = self.do_rpc_call(
            self.scheduler_cli, 'fn_one_zr_step_end',
            ret=ret)
        return r


    def fn_one_zr_step_fwd_weig_brcast(self, msg, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_WKR, '>>> fn_one_zr_step_fwd_weig_brcast')
        m = self.msg_unpck(msg)
        msg = m.get('msg') or {}

        self.q_put_state(Q_STATE_HEAD_ZR_WKR + 203, msg)

        rep = self.mk_rpc_ret('fn_one_zr_step_fwd_weig_brcast', 'ok')
        return self.msg_pck(rep)

    def do_one_zr_step_fwd_weig_brcast_resp(self, ret, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_WKR, '>>> do_one_zr_step_fwd_weig_brcast_resp')
        r = self.do_rpc_call(
            self.scheduler_cli, 'fn_one_zr_step_fwd_weig_brcast_resp',
            ret=ret)
        return r


    def fn_one_zr_step_calc_loss(self, msg, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_WKR, '>>> fn_one_zr_step_calc_loss')
        m = self.msg_unpck(msg)
        msg = m.get('msg') or {}

        self.q_put_state(Q_STATE_HEAD_ZR_WKR + 221, msg)

        rep = self.mk_rpc_ret('fn_one_zr_step_calc_loss', 'ok')
        return self.msg_pck(rep)

    def do_one_zr_step_loss_calced(self, ret, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_WKR, '>>> do_one_zr_step_loss_calced')
        r = self.do_rpc_call(
            self.scheduler_cli, 'fn_one_zr_step_loss_calced',
            ret=ret)
        return r


    def fn_one_zr_step_bcwd_grad_reduce(self, msg, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_WKR, '>>> fn_one_zr_step_bcwd_grad_reduce')
        m = self.msg_unpck(msg)
        msg = m.get('msg') or {}

        self.q_put_state(Q_STATE_HEAD_ZR_WKR + 223, msg)

        rep = self.mk_rpc_ret('fn_one_zr_step_bcwd_grad_reduce', 'ok')
        return self.msg_pck(rep)

    def do_one_zr_step_bcwd_grad_reduce_resp(self, ret, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_WKR, '>>> do_one_zr_step_bcwd_grad_reduce_resp')
        r = self.do_rpc_call(
            self.scheduler_cli, 'fn_one_zr_step_bcwd_grad_reduce_resp',
            ret=ret)
        return r


    def fn_one_zr_step_bcwd_weig_brcast(self, msg, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_WKR, '>>> fn_one_zr_step_bcwd_weig_brcast')
        m = self.msg_unpck(msg)
        msg = m.get('msg') or {}

        self.q_put_state(Q_STATE_HEAD_ZR_WKR + 225, msg)

        rep = self.mk_rpc_ret('fn_one_zr_step_bcwd_weig_brcast', 'ok')
        return self.msg_pck(rep)

    def do_one_zr_step_bcwd_weig_brcast_resp(self, ret, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_WKR, '>>> do_one_zr_step_bcwd_weig_brcast_resp')
        r = self.do_rpc_call(
            self.scheduler_cli, 'fn_one_zr_step_bcwd_weig_brcast_resp',
            ret=ret)
        return r


    def fn_one_zr_step_opt_one_step(self, msg, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_WKR, '>>> fn_one_zr_step_opt_one_step')
        m = self.msg_unpck(msg)
        msg = m.get('msg') or {}

        self.q_put_state(Q_STATE_HEAD_ZR_WKR + 241, msg)

        rep = self.mk_rpc_ret('fn_one_zr_step_opt_one_step', 'ok')
        return self.msg_pck(rep)

    def do_one_zr_step_opt_one_step_resp(self, ret, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_WKR, '>>> do_one_zr_step_opt_one_step_resp')
        r = self.do_rpc_call(
            self.scheduler_cli, 'fn_one_zr_step_opt_one_step_resp',
            ret=ret)
        return r
...
class MPSVCWorker(svctmp.TmplService):
    ...
    def check_state(self, *args, **kwargs):
        ...
        # ----------------- zero one step func -----------------
        ...
        elif svr_state == Q_STATE_HEAD_ZR_WKR + 201:
            ...
            msg = sa[0]
            ret = self.svr.trnr.check_msg(msg)
            r = self.svr.do_one_zr_step_ack(ret)


        elif svr_state == Q_STATE_HEAD_ZR_WKR + 203:

            msg = sa[0]
            ret = self.svr.trnr.check_msg(msg)
            r = self.svr.do_one_zr_step_fwd_weig_brcast_resp(ret)



        elif svr_state == Q_STATE_HEAD_ZR_WKR + 221:

            msg = sa[0]
            ret = self.svr.trnr.check_msg(msg)
            r = self.svr.do_one_zr_step_loss_calced(ret)


        elif svr_state == Q_STATE_HEAD_ZR_WKR + 223:

            msg = sa[0]
            ret = self.svr.trnr.check_msg(msg)
            r = self.svr.do_one_zr_step_bcwd_grad_reduce_resp(ret)


        elif svr_state == Q_STATE_HEAD_ZR_WKR + 225:

            msg = sa[0]
            ret = self.svr.trnr.check_msg(msg)
            r = self.svr.do_one_zr_step_bcwd_weig_brcast_resp(ret)


        elif svr_state == Q_STATE_HEAD_ZR_WKR + 241:

            msg = sa[0]
            ret = self.svr.trnr.check_msg(msg)
            r = self.svr.do_one_zr_step_opt_one_step_resp(ret)
...
class MPSVRScheduler(svctmp.TmplServer):
    def __init__(self, cfg={}):
        ...
        self.one_zr_step_flags = {}
        self.one_zr_step_order_i = 0
        self.one_zr_step_order_n = len(self.net_map.keys())  # NOTE: this will be self.world_size if use main_pg as zero_pg

        self.one_zr_step_weig_brcast_flags = {}
        self.one_zr_step_grad_reduce_flags = {}
        self.one_zr_step_loss_calc_flags = {}
        self.one_zr_step_opt_one_step_flags = {}
        ...
        self.zr_group = self.nccl_groups_grp_map.get('zr_group', [])
        if not self.zr_group:
            self.zr_group = self.nccl_group_main_grp_list
        self.zr_group_n = len(self.zr_group)
        self.zr_group_l = len(self.zr_group[0])
        print('zr_group: ', self.zr_group)
    ...
    def trainer_check_ret(self, ret, rank, role, *args, **kwargs):
        ...
        # -------------------------------------------------------
            ...
            if len(self.next_epoch_flags.keys()) >= self.kn:
                ...
                return Q_STATE_HEAD_ZR_SCH + 201, 'one_zr_step_start'
            else:
                return 0, 'wait'
        ...
        elif r == 'one_zr_step_started':
            self.one_zr_step_flags[k] = 1
            if len(self.one_zr_step_flags.keys()) >= self.kn:
                self.one_zr_step_flags = {}
                return Q_STATE_HEAD_ZR_SCH + 203, 'one_zr_step_fwd_weig_brcast'
            else:
                return 0, 'wait'


        elif r == 'one_zr_step_fwd_weig_brcast_resp':
            self.one_zr_step_weig_brcast_flags[k] = 1
            if len(self.one_zr_step_weig_brcast_flags.keys()) >= self.kn:
                self.one_zr_step_weig_brcast_flags = {}
                self.one_zr_step_order_i += 1  # next ZeRO part =====================>
                if self.one_zr_step_order_i < self.one_zr_step_order_n:  # i: --> n
                    return Q_STATE_HEAD_ZR_SCH + 203, 'one_zr_step_fwd_weig_brcast'  # next modi
                else:  # all part mods finish forward
                    self.one_zr_step_order_i -= 1  # i: --> n -1
                    return Q_STATE_HEAD_ZR_SCH + 221, 'one_zr_step_calc_loss'
            else:
                return 0, 'wait'


        elif r == 'one_zr_step_loss_calced':
            self.one_zr_step_loss_calc_flags[k] = 1
            if len(self.one_zr_step_loss_calc_flags.keys()) >= self.kn:
                self.one_zr_step_loss_calc_flags = {}
                return Q_STATE_HEAD_ZR_SCH + 223, 'one_zr_step_bcwd_grad_reduce'
            else:
                return 0, 'wait'


        elif r == 'one_zr_step_bcwd_weig_brcast_resp':
            self.one_zr_step_weig_brcast_flags[k] = 1
            if len(self.one_zr_step_weig_brcast_flags.keys()) >= self.kn:
                self.one_zr_step_weig_brcast_flags = {}
                return Q_STATE_HEAD_ZR_SCH + 223, 'one_zr_step_bcwd_grad_reduce'
        ...
        elif r == 'one_zr_step_bcwd_grad_reduce_resp':
            self.one_zr_step_grad_reduce_flags[k] = 1
            if len(self.one_zr_step_grad_reduce_flags.keys()) >= self.kn:
                self.one_zr_step_grad_reduce_flags = {}
                self.one_zr_step_order_i -= 1  # prev ZeRO part =====================>
                if self.one_zr_step_order_i < 0:  # all part mods finish backward and grad_reduce
                    self.one_zr_step_order_i += 1  # i: --> 0
                    return Q_STATE_HEAD_ZR_SCH + 241, 'one_zr_step_opt_one_step'  # opt_one_step
                else:
                    return Q_STATE_HEAD_ZR_SCH + 225, 'one_zr_step_bcwd_weig_brcast'
            else:
                return 0, 'wait'


        elif r == 'one_zr_step_opt_one_step_resp':
            self.one_zr_step_opt_one_step_flags[k] = 1
            if len(self.one_zr_step_opt_one_step_flags.keys()) >= self.kn:
                self.one_zr_step_opt_one_step_flags = {}
                return Q_STATE_HEAD_ZR_SCH + 201, 'one_zr_step_start'
            else:
                return 0, 'wait'
    ...
    def do_one_zr_step_start(self, cmd, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> do_one_zr_step_start')
        r = self.do_rpc_call_wkrs(
            'fn_one_zr_step_start',
            cmd=cmd, **kwargs)

        return r

    def fn_one_zr_step_ack(self, msg, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> fn_one_zr_step_ack')
        m = self.msg_unpck(msg)
        role = m.get('role')
        rank = m.get('rank')
        msg = m.get('msg') or {}
        ...
        self.q_put_state(Q_STATE_HEAD_ZR_SCH + 202, msg, rank, role)

        rep = self.mk_rpc_ret('fn_one_zr_step_ack', 'ok')
        return self.msg_pck(rep)

    def fn_one_zr_step_end(self, msg, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> fn_one_zr_step_end')
        m = self.msg_unpck(msg)
        role = m.get('role')
        rank = m.get('rank')
        msg = m.get('msg') or {}
        ...
        self.q_put_state(Q_STATE_HEAD_ZR_SCH + 210, msg, rank, role)

        rep = self.mk_rpc_ret('fn_one_zr_step_end', 'ok')
        return self.msg_pck(rep)


    def do_one_zr_step_fwd_weig_brcast(self, cmd, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> do_one_zr_step_fwd_weig_brcast')
        r = self.do_rpc_call_wkrs(
            'fn_one_zr_step_fwd_weig_brcast',
            cmd=cmd,
            one_zr_step_order_i=self.one_zr_step_order_i,
            **kwargs)

        return r

    def fn_one_zr_step_fwd_weig_brcast_resp(self, msg, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> fn_one_zr_step_fwd_weig_brcast_resp')
        m = self.msg_unpck(msg)
        role = m.get('role')
        rank = m.get('rank')
        msg = m.get('msg') or {}
        ...
        self.q_put_state(Q_STATE_HEAD_ZR_SCH + 204, msg, rank, role)

        rep = self.mk_rpc_ret('fn_one_zr_step_fwd_weig_brcast_resp', 'ok')
        return self.msg_pck(rep)


    def do_one_zr_step_fwd_modi_fwd(self, cmd, *args, **kwargs):  # NOTE: not used for now
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> do_one_zr_step_fwd_modi_fwd')
        r = self.do_rpc_call_wkrs(
            'fn_one_zr_step_fwd_modi_fwd',
            cmd=cmd,
            one_zr_step_order_i=self.one_zr_step_order_i,
            **kwargs)
    ...
    def do_one_zr_step_calc_loss(self, cmd, pre_ret, *args, **kwargs):  # for last mod
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> do_one_zr_step_calc_loss')
        r = self.do_rpc_call_wkrs(
            'fn_one_zr_step_calc_loss',
            cmd=cmd,
            one_zr_step_order_i=self.one_zr_step_order_i,
            **kwargs)
    ...
    def fn_one_zr_step_loss_calced(self, msg, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> fn_one_zr_step_loss_calced')
        m = self.msg_unpck(msg)
        role = m.get('role')
        rank = m.get('rank')
        msg = m.get('msg') or {}
        ...
        self.q_put_state(Q_STATE_HEAD_ZR_SCH + 222, msg, rank, role)

        rep = self.mk_rpc_ret('fn_one_zr_step_loss_calced', 'ok')
        return self.msg_pck(rep)


    def do_one_zr_step_bcwd_weig_brcast(self, cmd, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> do_one_zr_step_bcwd_weig_brcast')
        r = self.do_rpc_call_wkrs(
            'fn_one_zr_step_bcwd_weig_brcast',
            cmd=cmd,
            one_zr_step_order_i=self.one_zr_step_order_i,
            **kwargs)
    ...
    def fn_one_zr_step_bcwd_weig_brcast_resp(self, msg, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> fn_one_zr_step_bcwd_weig_brcast_resp')
        m = self.msg_unpck(msg)
        role = m.get('role')
        rank = m.get('rank')
        msg = m.get('msg') or {}
        ...
        self.q_put_state(Q_STATE_HEAD_ZR_SCH + 226, msg, rank, role)

        rep = self.mk_rpc_ret('fn_one_zr_step_bcwd_weig_brcast_resp', 'ok')
        return self.msg_pck(rep)


    def do_one_zr_step_bcwd_modi_bwd(self, cmd, *args, **kwargs):  # NOTE: not used for now
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> do_one_zr_step_bcwd_modi_bwd')
        r = self.do_rpc_call_wkrs(
            'fn_one_zr_step_bcwd_modi_bwd',
            cmd=cmd,
            one_zr_step_order_i=self.one_zr_step_order_i,
            **kwargs)
    ...
    def do_one_zr_step_bcwd_grad_reduce(self, cmd, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> do_one_zr_step_bcwd_grad_reduce')
        r = self.do_rpc_call_wkrs(
            'fn_one_zr_step_bcwd_grad_reduce',
            cmd=cmd,
            one_zr_step_order_i=self.one_zr_step_order_i,
            **kwargs)
    ...
    def fn_one_zr_step_bcwd_grad_reduce_resp(self, msg, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> fn_one_zr_step_bcwd_grad_reduce_resp')
        m = self.msg_unpck(msg)
        role = m.get('role')
        rank = m.get('rank')
        msg = m.get('msg') or {}
        ...
        self.q_put_state(Q_STATE_HEAD_ZR_SCH + 224, msg, rank, role)

        rep = self.mk_rpc_ret('fn_one_zr_step_bcwd_grad_reduce_resp', 'ok')
        return self.msg_pck(rep)


    def do_one_zr_step_opt_one_step(self, cmd, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> do_one_zr_step_opt_one_step')
        r = self.do_rpc_call_wkrs(
            'fn_one_zr_step_opt_one_step',
            cmd=cmd, **kwargs)

        return r

    def fn_one_zr_step_opt_one_step_resp(self, msg, *args, **kwargs):
        ilog.debug(ILOG_I_ZR_SVR_SCH, '>>> fn_one_zr_step_opt_one_step_resp')
        m = self.msg_unpck(msg)
        role = m.get('role')
        rank = m.get('rank')
        msg = m.get('msg') or {}
        ...
        self.q_put_state(Q_STATE_HEAD_ZR_SCH + 242, msg, rank, role)

        rep = self.mk_rpc_ret('fn_one_zr_step_opt_one_step_resp', 'ok')
        return self.msg_pck(rep)
...
class MPSVCScheduler(svctmp.TmplService):
    ...
    def check_state(self, *args, **kwargs):
        ...
        # ----------------- worker_reg -----------------
            ...
            #=#self.q_put_state(Q_STATE_HEAD_ZR_SCH + 201, 'one_zr_step_start')
        ...
        # ----------------- one_zero_dp_step_func >>> -----------------


        elif svr_state == Q_STATE_HEAD_ZR_SCH + 201:  # one_zr_step_start
            cmd = sa[0]
            r = self.svr.do_one_zr_step_start(cmd, typ=self.nccl_allreduce_typ)

        elif svr_state == Q_STATE_HEAD_ZR_SCH + 202:  # fn_one_zr_step_ack
            if sa:
                msg, rank, role = sa
                ret = msg.get('ret')
        ...
        elif svr_state == Q_STATE_HEAD_ZR_SCH + 203:  # one_zr_step_fwd_weig_brcast
            cmd = sa[0]
            r = self.svr.do_one_zr_step_fwd_weig_brcast(cmd, typ=self.nccl_allreduce_typ)

        elif svr_state == Q_STATE_HEAD_ZR_SCH + 204:  # fn_one_zr_step_fwd_weig_brcast_resp
            if sa:
                msg, rank, role = sa
                ret = msg.get('ret')
        ...
        elif svr_state == Q_STATE_HEAD_ZR_SCH + 221:  # one_zr_step_calc_loss
            cmd = sa[0]
            pre_ret = sa[1]
            r = self.svr.do_one_zr_step_calc_loss(cmd, pre_ret, typ=self.nccl_allreduce_typ)

        elif svr_state == Q_STATE_HEAD_ZR_SCH + 222:  # fn_one_zr_step_loss_calced
            if sa:
                msg, rank, role = sa
                ret = msg.get('ret')
        ...
        elif svr_state == Q_STATE_HEAD_ZR_SCH + 223:  # one_zr_step_bcwd_grad_reduce
            cmd = sa[0]
            pre_ret = sa[1]
            r = self.svr.do_one_zr_step_bcwd_grad_reduce(cmd, pre_ret, typ=self.nccl_allreduce_typ)

        elif svr_state == Q_STATE_HEAD_ZR_SCH + 224:  # fn_one_mp_step_grads_transed
            if sa:
                msg, rank, role = sa
                ret = msg.get('ret')
        ...
        elif svr_state == Q_STATE_HEAD_ZR_SCH + 225:  # one_zr_step_bcwd_weig_brcast
            cmd = sa[0]
            pre_ret = sa[1]
            r = self.svr.do_one_zr_step_bcwd_weig_brcast(cmd, pre_ret, typ=self.nccl_allreduce_typ)

        elif svr_state == Q_STATE_HEAD_ZR_SCH + 226:  # fn_one_zr_step_bcwd_weig_brcast_resp
            if sa:
                msg, rank, role = sa
                ret = msg.get('ret')
        ...
        elif svr_state == Q_STATE_HEAD_ZR_SCH + 241:  # one_zr_step_opt_one_step
            cmd = sa[0]
            pre_ret = sa[1]
            r = self.svr.do_one_zr_step_opt_one_step(cmd, pre_ret, typ=self.nccl_allreduce_typ)

        elif svr_state == Q_STATE_HEAD_ZR_SCH + 242:  # fn_one_zr_step_opt_one_step_resp
            if sa:
                msg, rank, role = sa
                ret = msg.get('ret')
