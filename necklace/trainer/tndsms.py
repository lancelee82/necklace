"""Distributed Model-Parallel (vertically splitted) Trainer Services (Scheduler + Worker)"""
...
class SVRWorker(svctmp.TmplServer):
    ...
    def fn_cre_nccl_nuid_pg(self, msg, *args, **kwargs):
        ilog.debug(500001, '>>> fn_cre_nccl_nuid_pg')
        m = self.msg_unpck(msg)
        msg = m.get('msg') or {}
    ...
    def fn_init_nccl_comm_pg(self, msg, *args, **kwargs):
        ilog.debug(500001, '>>> fn_init_nccl_comm_pg')
        m = self.msg_unpck(msg)
        msg = m.get('msg') or {}
...
class SVRScheduler(svctmp.TmplServer):
    ...
    def do_cre_nccl_nuid_pg(self, wkr_rank=0, cfg={}, *args, **kwargs):
        ilog.debug(500002, '>>> do_cre_nccl_nuid_pg')
        wkr = self.worker_url(wkr_rank)
        cli = self.worker_clis[wkr]
        r = self.do_rpc_call(
    ...
    def do_init_nccl_comm_pg(self, wkr_rank, nuid, pg_rank, world_size, pg_key, *args, **kwargs):
        ilog.debug(500002, '>>> do_init_nccl_comm_pg')
    ...
    def do_check_nccl_groups_cfg_map(self, *args, **kwargs):
        ilog.debug(500002, '>>> do_check_nccl_groups_cfg_map')
