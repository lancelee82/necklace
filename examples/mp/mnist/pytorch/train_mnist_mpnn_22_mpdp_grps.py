...
import necklace
from necklace.trainer import tndsms
...
# ----------------------------------------------------------------------------
from necklace.data import dtdummy
...
def get_data_loader(*args, **kwargs):
    ...
    # ============================================================
    # NOTE: when trainint with MP + DP,
    #       in DP group torch distributed sampler is used,
    #       in MP group the same data batch is used and broadcast
    #       with NCCL from rank-0 to others
    # ============================================================
    if mp_rank == 0:  # only rank-0 has real dataloader
        ...
        ds_len = 60000  # NOTE: we just know it :p
        train_dataset = dtdummy.DtstDummy(ds_len // dp_world_size)  # NOTE should same with DP
        train_loader = dtdummy.DtldDummy(train_dataset, batch_size, drop_last=True)

    # TODO: distributed Dataloader for MP+DP
...
if __name__ == '__main__':
    ...
    if role == 'scheduler':
        cfg = {
            ...
            'epochs': args.epochs,
            # ========================================================
            # NOTE: we do mp + dp at one_step,
            #       mp use at-model-design drived nccl fns,
            #       dp use rpc msg drived nccl fns
            # ========================================================
            'nccl_allreduce_typ': 'grad', #'oopt', #'weig'
        ...
        cfg = {
            ...
            'trainer_cfg': {
                ...
                'dataloader_type': 'mpdp',  # NOTE:
                'dataloader_data_shape_dims': [4,1],  # [64,1,28,28], [64]
                'dataloader_data_typs': [torch.float32,torch.long],
...
# python train_mnist_mpnn_22_mpdp_grps.py -r scheduler -w 4 -k 0 -t "dp+mp" -dpsz 2 -mpsz 2 --epochs 3 -u ipc:///tmp/snp-svr-1.ipc
# python train_mnist_mpnn_22_mpdp_grps.py -r worker -w 4 -k 0 -g 0 -t "dp+mp" -dpsz 2 -mpsz 2 -u ipc:///tmp/ngn-wkr-0.ipc -s ipc:///tmp/snp-svr-1.ipc
# python train_mnist_mpnn_22_mpdp_grps.py -r worker -w 4 -k 1 -g 1 -t "dp+mp" -dpsz 2 -mpsz 2 -u ipc:///tmp/ngn-wkr-1.ipc -s ipc:///tmp/snp-svr-1.ipc
# python train_mnist_mpnn_22_mpdp_grps.py -r worker -w 4 -k 2 -g 2 -t "dp+mp" -dpsz 2 -mpsz 2 -u ipc:///tmp/ngn-wkr-2.ipc -s ipc:///tmp/snp-svr-1.ipc
# python train_mnist_mpnn_22_mpdp_grps.py -r worker -w 4 -k 3 -g 3 -t "dp+mp" -dpsz 2 -mpsz 2 -u ipc:///tmp/ngn-wkr-3.ipc -s ipc:///tmp/snp-svr-1.ipc
